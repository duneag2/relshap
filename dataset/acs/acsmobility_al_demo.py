...
out_path = build_acs_duckdb(acs_data, out_path="./acs/acsmobility_al.duckdb")
...
DB_PATH = "./acs/acsmobility_al.duckdb"
...
SQL_PATH = "./acs/query_acsmobility_al_demo.sql"
...
con.close()
df = df.fillna(0) # acsmobility 어쩐일인지 nan을 다 0으로 대치함
df = df[df["MIG"] != 9].copy() # 9는 다 제거

df_original = df
df = df.drop(["SERIALNO", "SPORDER", "MIG"],axis=1)
...
df_original["MIG"] = (df_original["MIG"] != 1).astype(int) #target
...
flattened_al_demo.to_csv(
    "./acs/flattened_acsmobility_al_demo.csv",
    header=feature_list,
    index=False
)