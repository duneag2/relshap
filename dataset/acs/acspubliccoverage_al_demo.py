...
out_path = build_acs_duckdb(acs_data, out_path="./acs/acspubliccoverage_al.duckdb")
...
DB_PATH = "./acs/acspubliccoverage_al.duckdb"
...
SQL_PATH = "./acs/query_acspubliccoverage_al_demo.sql"
...
con.close()
df = df.fillna(0) # acspubliccoverage는 어쩐일인지 nan을 다 0으로 대치함

df_original = df
df = df.drop(["SERIALNO", "SPORDER", "PUBCOV"],axis=1)
...
# df_original["PUBCOV"] = (df_original["PUBCOV"] == 1).astype(int) # pubcov 추가 가공 없음
...
flattened_al_demo.to_csv(
    "./acs/flattened_acspubliccoverage_al_demo.csv",
    header=feature_list,
    index=False
)