...
out_path = build_acs_duckdb(acs_data, out_path="./acs/acstraveltime_al.duckdb")
...
DB_PATH = "./acs/acstraveltime_al.duckdb"
...
SQL_PATH = "./acs/query_acstraveltime_al_demo.sql"
...
con.close()
df = df.fillna(0) # acstraveltime은 어쩐일인지 nan을 다 0으로 대치함

df_original = df
df = df.drop(["SERIALNO", "SPORDER", "JWMNP"],axis=1)
...
df_original["JWMNP"] = (df_original["JWMNP"] > 20).astype(int) #target
...
flattened_al_demo.to_csv(
    "./acs/flattened_acstraveltime_al_demo.csv",
    header=feature_list,
    index=False
)